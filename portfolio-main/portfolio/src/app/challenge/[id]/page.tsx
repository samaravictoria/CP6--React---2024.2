...
import { TipoSprint, TipoAluno } from "@/types";
...
export default function Sprints({ params }: { params: { id: number } }) {
  const [sprint, setSprint] = useState<TipoSprint[]>([]);
  ...
  const chamadaApi = async () => {
    const responseSprint = await fetch(`/api/sprints/${params.id}`);
    const dadosSprint = await responseSprint.json();
    setSprint(dadosSprint);
  ...
  const handleAdicionarSprint = async () => {
    if (!materia || !semestre || !nota) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert("Erro ao adicionar sprint.");
      }
    } catch (error) {
    ...
    <div className="bg-black min-h-screen text-white">
      ...
      <div className="flex justify-start pt-3 px-6 space-x-6">
        ...
        <div className="flex-1 bg-[#ed195c] p-6 font-bold">
          <h2 className="text-2xl text-black mb-4">Notas das Sprints</h2>
          ...
          {showForm && (
            <div className="bg-[#1f1f1f] p-6 mb-6">
              <h3 className="text-xl font-semibold text-white mb-4">Adicionar Nova Sprint</h3>
